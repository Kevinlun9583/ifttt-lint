...
fn run_case(case: &CheckCase) {
    ...
    let result = check(&mock, &change_map, &matchers, 0).unwrap();
...
#[parameterized(
    ...
    // Multi-line block comments are intentionally not scanned for directives.
    // The scanner only extracts from single-line block comments (open + close
    // on the same line). This avoids false positives from directives mentioned
    // in explanatory block comments.
    multiline_block_comment_ignored = { CheckCase {
        files: files!{
            "test.js" => "
                /*
                 * LINT.IfChange
        ...
        changes: Some(const { &[sub("test.js", &[7])] }),
...
",
    ...
    prose_if_change_with_trailing_text_ignored = { CheckCase {
        files: files! {
            "test.rs" => "
                // LINT.IfChange(label) is used for marking regions
                // LINT.IfChange
                fn f() {}
                // LINT.ThenChange(//b.rs)
            ",
    ...
    prose_then_change_with_trailing_text_ignored = { CheckCase {
        files: files! {
            "test.rs" => "
                // LINT.ThenChange() marks the end of a region
                // LINT.IfChange
                fn f() {}
                // LINT.ThenChange(//b.rs)
            ",
...
#[parameterized(
    ...
    nested_labeled_if_change_rejected = { CheckCase {
        // Nesting is not supported — the outer IfChange is reported as unpaired.
        files: files!{
            "test.rs" => "
                // LINT.IfChange(outer)
                // LINT.IfChange(inner)
    ...
    malformed_label = { CheckCase {
        ...
        expected_errors: Some(&["malformed label"]),
        ..DEFAULTS
    } },
    ...
    unlabeled_if_change_empty_then_change = { CheckCase {
        // Unlabeled IfChange with empty ThenChange() is useless — error.
        files: files!{
            "test.rs" => "
                // LINT.IfChange
                code
        ...
        expected_errors: Some(&["empty LINT.ThenChange() on unlabeled IfChange has no effect"]),
        ..DEFAULTS
    } },
    labeled_if_change_empty_then_change_ok = { CheckCase {
        // Labeled IfChange with empty ThenChange() is valid — serves as target.
        files: files!{
            "source.rs" => "
                // LINT.IfChange
                code
                // LINT.ThenChange(//target.rs:my_label)
            ",
            "target.rs" => "
...
#[parameterized(
    ...
    empty_then_change_is_valid = { CheckCase {
        // Empty LINT.ThenChange() is valid — used for target-only regions.
        files: files!{
            "src/a.rs" => "
...
fn binary_file_skipped() {
    ...
    let result = check(&mock, &changes, &[], 0).unwrap();
...
#[parameterized(
    ...
    then_change_empty_parens = { CheckCase {
        // ThenChange() with empty parens on a *labeled* IfChange is valid.
        files: files! {
            "test.rs" => "
                // LINT.IfChange(target_label)
                int x;
                // LINT.ThenChange()
            ",
        },
...
#[parameterized(
    ...
    empty_parens_content_changed = { CheckCase {
        // ThenChange() with empty target list on a *labeled* IfChange: content
        // changes produce no finding because the block serves as a target.
        files: files! {
            "test.rs" => "
                // LINT.IfChange(my_label)
                int x;
...
#[parameterized(
    ...
    content_removal_collapsed_to_then_change_line = { CheckCase {
        // Known limitation: when a content line removal and a ThenChange
        // replacement both collapse to the same removed_new_pos (the
        // ThenChange line), is_triggered cannot distinguish them. The
        // removal_end cap excludes the ThenChange position, inadvertently
        // hiding the content removal. This documents the false negative.
        files: files!{
            "a.rs" => "
                // LINT.IfChange
                remaining
        ...
        // Content was removed AND ThenChange was replaced, but both
        // removals map to the same new-file position (3 = ThenChange line).
        // The tool treats position 3 as a ThenChange replacement, not a
        // content removal → false negative (no finding).
        changes: Some(&[("a.rs", &[3], &[3])]),
        expected_findings: Some(&[]),
        expected_finding_count: Some(0),
        ..DEFAULTS
    } },
...
/// C1: Verify that removed_new_positions from two diff patches targeting the
/// same file are both visible.  The udiff parser merges them, and the checker
/// must see both positions when deciding whether a guarded region was touched.
#[test]
fn removed_new_positions_merge_across_patches() {
    // Scenario: a.rs guards two regions.  Region 1 targets target.rs (line 2
    // removed via rename-patch), region 2 also targets target.rs (line 3
    // removed via direct patch).  Both removals must register so neither
    // region produces a false negative.
    let mut mock = MockVcsProvider::default();
    mock.add_file(
        "a.rs",
        &unindent(
    ...
    let change_map: ChangeMap = [(
        "target.rs".to_string(),
        FileChanges {
            // Simulate merged result of two patches: removals at new-file
            // positions 2 and 3.
            removed_new_positions: [2, 3].iter().copied().collect(),
            removed_lines: [2, 3].iter().copied().collect(),
            ..Default::default()
        },
    )]
    .into_iter()
    ...
    let result = check(&mock, &change_map, &[], 0).unwrap();
    let warnings: Vec<_> = result
        .iter()
        .filter(|d| d.severity == Severity::Warning)
        .collect();

    // a.rs guards lines 1-3.  target.rs has removals at positions 2 and 3,
    // which overlap the guarded range → the check should pass (both sides
    // changed).
    assert!(
        warnings.is_empty(),
        "expected no findings (both sides changed), got: {warnings:?}"
    );
}

/// N2: A ThenChange line with an unclosed paren is parsed as a real directive
/// (with the paren and trailing comment as part of the target), not skipped
/// as prose.
#[test]
fn unclosed_paren_then_change_is_directive_not_prose() {
    let case = CheckCase {
        ...
        // The parser sees `( // trailing comment` as malformed.  We expect
        // an error, confirming it was NOT silently ignored as prose.
        expected_errors: Some(&["malformed directive"]),
        ..DEFAULTS
    };
    run_case(&case);
}
