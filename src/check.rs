...
/// Default thread count for parallel validation. File I/O contention on most
/// OSes makes higher counts counterproductive (benchmarked on Chromium and
/// TensorFlow).
pub const DEFAULT_THREADS: usize = 2;

#[must_use = "check result contains diagnostics that should be reported"]
pub fn check(
    vcs: &dyn VcsProvider,
    changes: &ChangeMap,
    ...
    if num_threads > 0 {
        ...
        pool.install(|| run_check(vcs, changes, ignore_patterns))
    } else {
        run_check(vcs, changes, ignore_patterns)
    }
}
...
fn parse_initial_cache(vcs: &dyn VcsProvider, scope: &ValidationScope) -> Result<ParseCache> {
...
fn collect_seed_parse_errors(
    ...
    for file_str in &scope.validate_files {
        ...
        // Skip files already reported by the changed_files loop above.
        if changes.get(file_str).is_some_and(|c| !c.deleted) {
            continue;
        }
...
///
/// Safety invariant: `cache` is only read (never mutated) during the
/// parallel `par_iter` phase.  Mutation happens sequentially via `extend`
/// after the parallel collect completes.
fn parse_files(
    paths: &[String],
    vcs: &dyn VcsProvider,
    ...
    let new_entries: Vec<(String, ParsedFile)> = paths
        ...
        .map(|rel_path| -> Result<Option<(String, ParsedFile)>> {
            Ok(parse_single_file(rel_path, vcs)?.map(|p| (rel_path.clone(), p)))
        })
...
/// Returns `Ok(None)` when the file does not exist (vs "unreadable" or "empty").
fn parse_single_file(rel_path: &str, vcs: &dyn VcsProvider) -> Result<Option<ParsedFile>> {
    let content = match vcs.read_file(rel_path)? {
        Some(FileContent::Text(c)) => c,
        Some(FileContent::Binary) => return Ok(Some(ParsedFile::empty())),
        None => return Ok(None),
    };

    if !content.contains("LINT.") {
        return Ok(Some(ParsedFile::empty()));
    }

    let (directives, mut errors) = crate::parser::parse(&content, rel_path);
    errors.extend(validate_uniqueness(&directives, rel_path));

    Ok(Some(ParsedFile { directives, errors }))
}
    ...
    for source in source_files {
        ...
        for directive in &parsed.directives {
            ...
            for target in targets {
                ...
                if !vcs.file_exists(&path).unwrap_or(false) {
                    continue;
                }
...
fn validate_structure(
    ...
    let mut errors = Vec::new();
    let mut pending_if: Option<(NonZeroUsize, bool)> = None;

    for d in directives {
        match d {
            Directive::IfChange { line, label } => {
                if let Some((prev_line, _)) = pending_if {
                ...
                pending_if = Some((*line, label.is_some()));
            }
            Directive::ThenChange { line, targets } => {
                if let Some((_, has_label)) = pending_if {
                    if targets.is_empty() && !has_label {
                        errors.push(mk_error(
                            file_path,
                            *line,
                            "empty LINT.ThenChange() on unlabeled IfChange has no effect"
                                .to_string(),
                        ));
                    }
                } else {
...
fn is_triggered(pair: &DirectivePair, sorted: Option<&SortedLines>) -> bool {
    ...
    let content_end = pair.then_line.get().saturating_sub(1);

    // Adjacent or overlapping directives (e.g. IfChange on line N, ThenChange
    // on line N+1) have no content lines — nothing can be triggered.
...
struct CheckContext<'a> {
    ...
    /// NOTE: benign TOCTOU race — two threads may miss the cache for the same
    /// path, both stat, and both insert. This is harmless (same result,
    /// last-write-wins with identical value) but worth noting if the cache
    /// logic is ever refactored to carry mutable state per entry.
...
fn resolve_target(
    ...
    {
        let cache = ctx.exists_cache.lock().expect("exists_cache poisoned");
...
fn scan_stale_references(
    ...
    ctx: &StaleReferenceCtx<'_>,
    cache: &ParseCache,
) -> Result<Vec<(Vec<Diagnostic>, CacheInsert)>> {
    candidates
        .par_iter()
        .map(
            |rel_str| -> Result<Option<(Vec<Diagnostic>, CacheInsert)>> {
                ...
                let Some((pairs, new_entry)) = stale_reference_scan_input(rel_str, ctx, cache)?
                else {
                    return Ok(None);
                };
...
fn stale_reference_scan_input(
    ...
    ctx: &StaleReferenceCtx<'_>,
    cache: &ParseCache,
) -> Result<Option<(Vec<DirectivePair>, CacheInsert)>> {
