...
//   2. Pick skip patterns: NO_SKIP, SkipPattern::Fixed { open, close }, or
...
static FALLBACK: Language = Language {
    ...
    // Scala, Elixir, …) to be a safe default for unknown extensions. Single-line
    // string literals ("…" / '…') are already handled by scan_for_openers.
    skip_patterns: TRIPLE_QUOTE_SKIP,
};
...
const NO_SKIP: &[SkipPattern] = &[];

const BACKTICK_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(BacktickString::try_open)];

const BACKTICK_FIXED_SKIP: &[SkipPattern] = &[SkipPattern::Fixed {
    open: "`",
    close: "`",
}];

const CPP_RAW_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(CppRaw::try_open)];
...
const DOLLAR_QUOTE_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(DollarQuote::try_open)];

const FENCED_CODE_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(FencedCode::try_open)];
...
const HEREDOC_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(Heredoc::try_open)];
...
const HTML_BLOCK_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(HtmlBlock::try_open)];

const LONG_BRACKET_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(LongBracket::try_open)];

const NIX_INDENT_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(NixIndent::try_open)];

const PHP_HEREDOC_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(PhpHeredoc::try_open)];

const PS_HERESTRING_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(PsHereString::try_open)];

const R_RAW_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(RRaw::try_open)];

const RUST_RAW_SKIP: &[SkipPattern] = &[SkipPattern::Dynamic(RustRaw::try_open)];
...
const TRIPLE_DOUBLE_SKIP: &[SkipPattern] = &[SkipPattern::Fixed {
    open: "\"\"\"",
    close: "\"\"\"",
}];
